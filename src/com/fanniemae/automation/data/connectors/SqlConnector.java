...
public class SqlConnector extends DataConnector {
	...
	public Boolean open() {
        try {
            ...
            // Issue 17118, 17120 - Set command timeout if any value other than empty string.
            ...
            } else if (_bUseMaxRows) {
                _pstmt.setFetchSize(_nMaxRows);
            }

            _rs = _pstmt.executeQuery();

            ResultSetMetaData rsmd = _rs.getMetaData();
            //_rs.getMetaData().getColumnCount();
            _ColumnCount = rsmd.getColumnCount();
            _nFinalColumnCount = _ColumnCount;
            _aFieldNames = new String[_ColumnCount];
            _aFieldTypes = new DataType[_ColumnCount];
            _aDataSchema = new String[_ColumnCount][2];
            ...
            for (int i = 0; i < _aFieldNames.length; i++) {
                ...
                //sName = System.Xml.XmlConvert.EncodeName(sName); //Issue 11872 - encode name if it contains illegal characters.
                String sTarget = String.format(";%s;", sName).toLowerCase();
                if (sUsedFieldNames.contains(sTarget)) {
                    //Issue 11808 - Under new system duplicate columns must be included with a unique name.
                    //              The code will add a sequenctial number to the field name until it is unique - or use GUID.
                ...
                _aDataSchema[nFieldNumber][0] = sName;
                _aFieldNames[nFieldNumber] = sName;
                _aDataSchema[nFieldNumber][1] = rsmd.getColumnClassName(i + 1);
                nFieldNumber++;
            }
        } catch (NumberFormatException | SQLException ex) {
            throw new Exception("Error while trying to open database connection. " + ex.getMessage(), ex);
        }
	...
	public Object[] getDataRow() {
	 ...
	 protected boolean NotPostgreSQL() throws SQLException {
	    ...
	    protected void AddCommandParameters() throws SQLException {
	        // Add paramters in the order listed.
	        ...
	        if ((nlParameters != null) && (nlParameters.getLength() > 0)) {
	            ...
	            for (int i = 0; i < iLength; i++) {
	                int iParam = i + 1;
	                //String sName = ((Element) nlParameters.item(i)).getAttribute("Name").trim();
	                ...
	                if (sValue.equals(sNullValue)) {
	                    _pstmt.setNull(iParam, Conversions.InfoDbTypeToJavaDbType(sParamType));
	                }

	                switch (Conversions.InfoDbTypeToJavaDbType(sParamType)) {
	                    case Types.BIGINT:
